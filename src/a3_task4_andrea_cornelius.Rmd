...
---
# Overview: In this code, I wrangle then visualize 
...
naples_purple_2010 <- urchins_clean %>% # Pull from clean urchins data
  ...
  filter(year_new == "2010") # Filter by year 
```


```{r}
ggplot(data = naples_purple_2010, aes(x = size, fill = treatment)) +
  ...
  labs(x = "Size",
       y = "Urchin Count",
       title = "Urchin Counts by Size and Treatment (CORNELIUS)") +
  theme_bw()
```
...
```{r}
naples_purple_2010_summary <- naples_purple_2010 %>%
  group_by(treatment) %>%
